...
let bonusSpinCount = parseInt(localStorage.getItem('ilove_spins') ?? '0');
if (!localStorage.getItem('ilove_first_visit')) {
    localStorage.setItem('ilove_first_visit', 'done');
    bonusSpinCount = 3;
    localStorage.setItem('ilove_spins', '3');
}
...
function showMemberBonus(memberName, color1, color2) {
    ...
    popup.innerHTML = `
        <div class="bonus-popup-inner" style="--bonus-color: ${color1}">
            ...
            <div class="bonus-spin-hint">GACHA TICKET +1</div>
        </div>
    `;
    ...
    // Award a slot spin
    bonusSpinCount++;
    localStorage.setItem('ilove_spins', bonusSpinCount.toString());
    updateSlotTicketDisplay();
...
// ==================== SLOT / GACHA SYSTEM（大幅強化） ====================
const slotPrizes = [
    // COMMON（50%）
    { emoji: '♥', label: 'LOVE運UP!', rarity: 'common', desc: '今日のあなたのLOVE運は最高！', effect: 'none' },
    { emoji: '♪', label: 'マイクGET!', rarity: 'common', desc: 'カラオケで=LOVEを歌おう！', effect: 'none' },
    { emoji: '✿', label: '桜吹雪', rarity: 'common', desc: '美しい桜がページを彩る', effect: 'sakura' },
    { emoji: '📣', label: 'コール練習', rarity: 'common', desc: 'MIX打ちの練習ができる！', effect: 'none' },
    { emoji: '🎪', label: 'グッズ引換券', rarity: 'common', desc: 'イコラブグッズをゲット（仮想）！', effect: 'none' },
    // RARE（30%）
    { emoji: '★', label: 'スター誕生', rarity: 'rare', desc: 'あなたもアイドル級の輝き！', effect: 'sparkle' },
    { emoji: '♫', label: 'メロディ', rarity: 'rare', desc: '=LOVEの曲が頭から離れない', effect: 'none' },
    { emoji: '♛', label: 'プロデューサー', rarity: 'rare', desc: '指原Pに認められた！', effect: 'none' },
    { emoji: '🌸', label: '春の息吹', rarity: 'rare', desc: 'ペンライトの花が咲き誇る', effect: 'sakura' },
    { emoji: '🎤', label: 'ステージパス', rarity: 'rare', desc: 'バックステージへのパスGET！', effect: 'none' },
    // SR（15%）
    { emoji: '🏟', label: '国立チケット', rarity: 'SR', desc: '国立競技場のチケットGET（仮想）！', effect: 'confetti' },
    { emoji: '💎', label: 'ダイヤモンドリリー', rarity: 'SR', desc: '探せ ダイヤモンドリリー発見！', effect: 'sparkle' },
    { emoji: '🎵', label: '限定MV', rarity: 'SR', desc: 'シークレットMVのアクセス権！', effect: 'confetti' },
    { emoji: '👑', label: 'ティアラ', rarity: 'SR', desc: '=LOVEの王冠を手に入れた！', effect: 'sparkle' },
    { emoji: '🌟', label: 'センターポジション', rarity: 'SR', desc: '今日はあなたがセンター！', effect: 'confetti' },
    // SSR（4%）
    { emoji: '∞', label: '奇跡の出会い', rarity: 'SSR', desc: '=LOVEとの運命的な出会い！', effect: 'rainbow' },
    { emoji: '=', label: 'レインボー', rarity: 'SSR', desc: '全メンバーカラーが虹のように輝く！', effect: 'rainbow' },
    { emoji: '🏆', label: 'レコード大賞', rarity: 'SSR', desc: '第67回 輝く！日本レコード大賞級の輝き！', effect: 'rainbow' },
    // UR（1%）極レア
    { emoji: '💖', label: '=LOVE永遠', rarity: 'UR', desc: 'あなたは永遠のイコラバーに認定されました！背景がスペシャルモードに！', effect: 'ultimate' },
    { emoji: '🗼', label: '国立制覇', rarity: 'UR', desc: 'MUFGスタジアム（国立競技場）完全制覇！全エフェクトMAX！', effect: 'ultimate' },
];

// ガチャ履歴保存
let gachaHistory = JSON.parse(localStorage.getItem('ilove_gacha_history') || '[]');
let gachaTotal = parseInt(localStorage.getItem('ilove_gacha_total') || '0');

function getGachaPity() {
    // 天井：50回でSR以上確定、100回でSSR以上確定
    const sinceLastSR = gachaTotal - (parseInt(localStorage.getItem('ilove_last_sr') || '0'));
    const sinceLastSSR = gachaTotal - (parseInt(localStorage.getItem('ilove_last_ssr') || '0'));
    return { sinceLastSR, sinceLastSSR };
}

function updateSlotTicketDisplay() {
    const badge = document.getElementById('slot-ticket-count');
    if (badge) badge.textContent = bonusSpinCount;
    const inner = document.getElementById('slot-ticket-count-inner');
    if (inner) inner.textContent = bonusSpinCount;
}

function spinSlot() {
    if (bonusSpinCount <= 0) {
        showSlotMessage('チケットがありません！\nメンバーカードをクリックして獲得しよう！');
        return;
    }
    bonusSpinCount--;
    localStorage.setItem('ilove_spins', bonusSpinCount.toString());
    updateSlotTicketDisplay();
    gachaTotal++;
    localStorage.setItem('ilove_gacha_total', gachaTotal.toString());

    const slotOverlay = document.getElementById('slot-overlay');
    if (!slotOverlay) return;
    slotOverlay.classList.add('active');

    // 天井（Pity）システム付きレアリティ判定
    const pity = getGachaPity();
    const roll = Math.random();
    let rarity;
    if (pity.sinceLastSSR >= 100) {
        rarity = 'SSR'; // 100回天井
    } else if (pity.sinceLastSR >= 50) {
        rarity = roll < 0.3 ? 'SSR' : 'SR'; // 50回天井
    } else if (roll < 0.01) {
        rarity = 'UR';
    } else if (roll < 0.05) {
        rarity = 'SSR';
    ...
    if (rarity === 'SR' || rarity === 'SSR' || rarity === 'UR') {
        localStorage.setItem('ilove_last_sr', gachaTotal.toString());
    }
    if (rarity === 'SSR' || rarity === 'UR') {
        localStorage.setItem('ilove_last_ssr', gachaTotal.toString());
    }

    const candidates = slotPrizes.filter(p => p.rarity === rarity);
    const prize = candidates[Math.floor(Math.random() * candidates.length)];

    // Animate the 3 slot reels
    const reels = slotOverlay.querySelectorAll('.slot-reel');
    const allEmojis = slotPrizes.map(p => p.emoji);

    reels.forEach((reel, i) => {
        reel.classList.add('spinning');
        let spins = 0;
        const maxSpins = 15 + i * 8;
        const interval = setInterval(() => {
            reel.textContent = allEmojis[Math.floor(Math.random() * allEmojis.length)];
            spins++;
            if (spins >= maxSpins) {
                clearInterval(interval);
                reel.classList.remove('spinning');
                reel.textContent = prize.emoji;
                reel.classList.add('stopped');
                // When last reel stops, show result
                if (i === reels.length - 1) {
                    setTimeout(() => {
                        showSlotResult(prize, slotOverlay);
                        updatePityDisplay();
                    }, 400);
                }
            }
...
function showSlotResult(prize, overlay) {
    const resultEl = overlay.querySelector('.slot-result');
    if (!resultEl) return;
    const rarityColors = { common: '#ffffff', rare: '#00bfff', SR: '#ffd700', SSR: '#ff1493', UR: '#ff0000' };
    const rarityBgs = { common: '', rare: 'linear-gradient(135deg,rgba(0,100,255,0.1),transparent)', SR: 'linear-gradient(135deg,rgba(255,215,0,0.15),transparent)', SSR: 'linear-gradient(135deg,rgba(255,20,147,0.2),transparent)', UR: 'linear-gradient(135deg,rgba(255,0,0,0.3),rgba(255,215,0,0.2),transparent)' };
    resultEl.innerHTML = `
        <div class="slot-result-rarity" style="color: ${rarityColors[prize.rarity]};
            font-size: ${prize.rarity === 'UR' ? '28px' : prize.rarity === 'SSR' ? '22px' : '18px'};
            text-shadow: 0 0 15px ${rarityColors[prize.rarity]};">${prize.rarity.toUpperCase()}</div>
        <div class="slot-result-emoji" style="font-size: ${prize.rarity === 'UR' ? '64px' : '48px'};
            ${prize.rarity === 'UR' ? 'animation: pulse 0.5s infinite;' : ''}">${prize.emoji}</div>
        <div class="slot-result-label">${prize.label}</div>
        <div class="slot-result-desc">${prize.desc}</div>
        <div style="margin-top:8px;font-size:11px;color:rgba(255,255,255,0.5);">通算 ${gachaTotal} 回目</div>
    `;
    resultEl.style.background = rarityBgs[prize.rarity];
    resultEl.classList.add('active');

    // 履歴保存
    gachaHistory.push({ rarity: prize.rarity, label: prize.label, time: Date.now() });
    if (gachaHistory.length > 100) gachaHistory = gachaHistory.slice(-100);
    localStorage.setItem('ilove_gacha_history', JSON.stringify(gachaHistory));

    // レアリティ別エフェクト
    if (prize.rarity === 'UR') {
        createUREffect();
    } else if (prize.rarity === 'SSR') {
        createSSREffect();
    } else if (prize.rarity === 'SR') {
        createSREffect();
    }
    // 特殊効果
    if (prize.effect === 'sakura') {
        // 桜をバーストさせる
        sakuraPetals.forEach(p => { p.position.y = 12 + Math.random() * 5; });
    } else if (prize.effect === 'rainbow' || prize.effect === 'ultimate') {
        triggerRainbowMode();
    }
}

function createUREffect() {
    // 画面全体が金色に輝く
    const flash = document.createElement('div');
    flash.style.cssText = `
        position:fixed;inset:0;z-index:10000;pointer-events:none;
        background:radial-gradient(ellipse at center,rgba(255,215,0,0.6),rgba(255,0,0,0.3),transparent);
        animation: songFlash 2s ease-out forwards;
    `;
    document.body.appendChild(flash);
    setTimeout(() => flash.remove(), 2000);
    // 大量のレインボー紙吹雪
    const colors = ['#ff1493', '#ff69b4', '#ffd700', '#00bfff', '#00ff7f', '#ff4500', '#9400d3', '#ff0000'];
    for (let i = 0; i < 100; i++) {
        const c = document.createElement('div');
        c.className = 'member-confetti';
        c.style.cssText = `
            left: ${Math.random() * 100}%; top: ${Math.random() * 30}%;
            background: ${colors[i % colors.length]};
            --conf-x: ${(Math.random() - 0.5) * 600}px;
            --conf-y: ${Math.random() * 500}px;
            --conf-r: ${Math.random() * 1440}deg;
...
function createSSREffect() {
    const flash = document.createElement('div');
    flash.className = 'ssr-flash';
    document.body.appendChild(flash);
    setTimeout(() => flash.remove(), 1500);
    // Rainbow confetti
    const colors = ['#ff1493', '#ff69b4', '#ffd700', '#00bfff', '#00ff7f', '#ff4500'];
    for (let i = 0; i < 60; i++) {
        const c = document.createElement('div');
        c.className = 'member-confetti';
        c.style.cssText = `
            ...
            background: ${colors[i % colors.length]};
            --conf-x: ${(Math.random() - 0.5) * 500}px;
            --conf-y: ${Math.random() * 300}px;
            --conf-r: ${Math.random() * 1080}deg;
...
function createSREffect() {
    const flash = document.createElement('div');
    flash.className = 'sr-flash';
    document.body.appendChild(flash);
...
function closeSlot() {
    ...
    if (overlay) {
        overlay.classList.remove('active');
        const result = overlay.querySelector('.slot-result');
        if (result) result.classList.remove('active');
        overlay.querySelectorAll('.slot-reel').forEach(r => {
...
// Slot button
document.getElementById('slot-btn')?.addEventListener('click', spinSlot);
document.getElementById('slot-overlay-close')?.addEventListener('click', closeSlot);
document.getElementById('slot-overlay')?.addEventListener('click', (e) => {
    if (e.target.id === 'slot-overlay') closeSlot();
});
// Make functions available for inline onclick (module scope)
window.spinSlot = spinSlot;
window.closeSlot = closeSlot;
window.toggleCollection = toggleCollection;
// Init ticket display + pity info
updateSlotTicketDisplay();
updatePityDisplay();

// ==================== GACHA COLLECTION SYSTEM ====================
function toggleCollection() {
    const collDiv = document.getElementById('gacha-collection');
    if (!collDiv) return;
    const isVisible = collDiv.style.display !== 'none';
    collDiv.style.display = isVisible ? 'none' : 'block';
    if (!isVisible) renderCollection();
}
...
function renderCollection() {
    ...
    // 統計
    const counts = { common: 0, rare: 0, SR: 0, SSR: 0, UR: 0 };
    const obtained = new Set();
    gachaHistory.forEach(h => {
        counts[h.rarity] = (counts[h.rarity] || 0) + 1;
        obtained.add(h.label);
    });
    const totalPrizes = slotPrizes.length;
    const completedCount = slotPrizes.filter(p => obtained.has(p.label)).length;

    stats.innerHTML = `
        <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin-bottom:12px;">
            <span style="color:#888;">通算: ${gachaTotal}回</span>
            <span style="color:#fff;">COMMON: ${counts.common}</span>
            <span style="color:#00bfff;">RARE: ${counts.rare}</span>
            <span style="color:#ffd700;">SR: ${counts.SR}</span>
            <span style="color:#ff1493;">SSR: ${counts.SSR}</span>
            <span style="color:#ff0000;">UR: ${counts.UR}</span>
        </div>
        <div style="font-size:12px;color:#ff69b4;">コンプリート: ${completedCount}/${totalPrizes} (${Math.round(completedCount/totalPrizes*100)}%)</div>
        <div style="width:100%;height:6px;background:rgba(255,255,255,0.1);border-radius:3px;margin:8px 0;overflow:hidden;">
            <div style="width:${completedCount/totalPrizes*100}%;height:100%;background:linear-gradient(90deg,#ff1493,#ffd700);border-radius:3px;transition:width 0.5s;"></div>
        </div>
    `;

    // プライズグリッド
    grid.innerHTML = '';
    slotPrizes.forEach(p => {
        const got = obtained.has(p.label);
        const card = document.createElement('div');
        const rarityColors = { common: '#666', rare: '#00bfff', SR: '#ffd700', SSR: '#ff1493', UR: '#ff0000' };
        card.style.cssText = `
            display:flex;flex-direction:column;align-items:center;padding:8px;border-radius:8px;
            background:${got ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.3)'};
            border:1px solid ${got ? rarityColors[p.rarity] : 'rgba(255,255,255,0.1)'};
            opacity:${got ? '1' : '0.4'};transition:all 0.3s;cursor:default;
            ${got ? 'box-shadow:0 0 8px ' + rarityColors[p.rarity] + '40;' : ''}
        `;
        card.innerHTML = `
            <div style="font-size:${p.rarity === 'UR' ? '28px' : '22px'};margin-bottom:4px;">${got ? p.emoji : '❓'}</div>
            <div style="font-size:10px;color:${rarityColors[p.rarity]};font-weight:bold;">${p.rarity}</div>
            <div style="font-size:10px;color:#ccc;text-align:center;margin-top:2px;">${got ? p.label : '???'}</div>
        `;
...
function updatePityDisplay() {
    const el = document.getElementById('slot-pity-info');
    if (!el) return;
    const pity = getGachaPity();
    el.innerHTML = `
        <span style="color:rgba(255,255,255,0.4);font-size:10px;">
            SR天井まで: ${Math.max(0, 50 - pity.sinceLastSR)}回 | SSR天井まで: ${Math.max(0, 100 - pity.sinceLastSSR)}回
        </span>
    `;
}
